...
# Open sheet by ID (safer than using the name "FPL Data")
...
def create_Gw_transfers_in_table(position_name):
    return player_df[player_df['Position'] == position_name][[
        'Player Name','Availability', 'Team', 'Cost', 'Form', 'FD Index', 'Clean Sheets', 'Goals', 'Assists', 'XG Current GW','XG Previous GW', 'ΔG_GW', 'XG', 'Delta G', 'XA', 'Delta GI', 'XG/90', 'Ownership (%)', 'GW Points', 'Expected points Next GW', 'Total Points', 'Difficulty Score', 
...
creds = ServiceAccountCredentials.from_json_keyfile_name("fpl-automation-456304-a599d109979b.json", scope)
client = gspread.authorize(creds)
sheet = client.open("FPL Data")
...
#def create_Gw_transfers_table(position_name):
 #   return player_df[player_df['Position'] == position_name][[
  #      'First Name', 'Last Name', 'Team', 'Cost', 'Form', 'FD Index', 'GW Points', 'Total Points','Manager Win', 'Manager Draw', 'Manager Loss', 'Manager Underdog Win', 'Manager Underdog draw',
   #     'Manager Cleansheet', 'Manager Goals scored', 'Difficulty Score'
    #] + [col for gw in next_5_gameweeks for col in [f'{gw["name"]}', f'{gw["name"]} Difficulty']] + [
     #   'GW Transfers In', 'GW Transfers Out'
    #]].sort_values(by='GW Transfers In', ascending=False)
